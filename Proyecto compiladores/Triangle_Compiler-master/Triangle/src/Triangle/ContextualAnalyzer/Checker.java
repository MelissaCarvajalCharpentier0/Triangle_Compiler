...
public final class Checker implements Visitor {
  ...
  public Object visitCaseCommand(CaseCommand ast, Object o) {
    // I changed everything that said IntegerLiteral to Terminal to make accept
    // IntergerLiteral and CharLiteral, in other words make generic
    idTable.openScope();
    ast.V.visit(this, null);
    if (ast.V.type != StdEnvironment.integerType && ast.V.type != StdEnvironment.charType) {
      reporter.reportError("CC incompatible Vname type (Integer or Character expected)", "", ast.position);
    }
    LinkedHashMap<Terminal, Command> MAP = ast.MAP;
    ArrayList<Terminal> AL = new ArrayList<Terminal>();
    for(Terminal T : MAP.keySet()){
      for(Terminal T2 : AL){
        if (T2.spelling.equals(T.spelling)){
          reporter.reportError("CC re-used terminal literal in case", "", ast.position);
        }
      }
      Command C = MAP.get(T);
      C.visit(this, null);
  ...
  public Object visitArrayExpression(ArrayExpression ast, Object o) {
    ...
    IntegerLiteral il = new IntegerLiteral(Integer.toString(ast.AA.elemCount), ast.position);
  ...
  public Object visitCaseExpression(CaseExpression ast, Object o){
      // I changed everything that said IntegerLiteral to Terminal to make accept
      // IntergerLiteral and CharLiteral, in other words generic
      idTable.openScope();
      ast.V.visit(this, null);
      if (ast.V.type != StdEnvironment.integerType && ast.V.type != StdEnvironment.charType) {
        reporter.reportError("CE incompatible Vname type (Integer or Character expected)", "", ast.position);
      }


    LinkedHashMap<Terminal, Expression> MAP = ast.MAP;
    ArrayList<Terminal> AL = new ArrayList<Terminal>();
    TypeDenoter caseType = null;
    for(Terminal T : MAP.keySet()){
      for(Terminal T2 : AL){
        if (T2.spelling.equals(T.spelling)){
          reporter.reportError("CE re-used terminal literal in case", "", ast.position);
        }
      }

      for (Expression expr : ast.MAP.values()) {
        TypeDenoter exprType = (TypeDenoter) expr.visit(this, null);
        if (caseType == null) {
          caseType = exprType;
        } else if (!caseType.equals(exprType)) {
          reporter.reportError("Case expression branches must have the same type", "", expr.position);
        }
      }
      Expression E = MAP.get(T);
      E.visit(this, null);
      T.visit(this, null);
      AL.add(T);
    }
