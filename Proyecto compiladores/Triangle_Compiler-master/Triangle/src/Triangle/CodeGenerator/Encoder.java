...
public final class Encoder implements Visitor {
  ...
  public Object visitCaseCommand(CaseCommand ast, Object o) {
        ...
        for(Terminal IL : ast.MAP.keySet()){
            ast.V.visit(this, frame);
            if (IL instanceof CharacterLiteral) {
              emit(Machine.LOADLop, 0, 0, (int) IL.spelling.charAt(0));
            } else {
  ...
  public Object visitCaseExpression(CaseExpression ast, Object o) {
    //Hecho por el chapas para que me dejara compilar
    Frame frame = (Frame) o;

    // Variable para rastrear el tamaño máximo de una rama.
    int maxBranchSize = 0;

    // Dirección inicial para el caso.
    int switchAddr = nextInstrAddr;

    // Iterar sobre las claves y valores en el mapa del caso.
    for (Terminal IL : ast.MAP.keySet()) {
      ast.V.visit(this, frame); // Visitar la variable evaluada en el "case".
      emit(Machine.LOADop, 1, Machine.STr, 0);

      // Cargar la constante del caso (Character o Integer literal).
      if (IL instanceof CharacterLiteral) {
        emit(Machine.LOADLop, 0, 0, (int) IL.spelling.charAt(0));
      } else {
        emit(Machine.LOADLop, 0, 0, Integer.parseInt(IL.spelling));
      ...
      // Visitar la expresión correspondiente al caso.
      int branchSize = (Integer) ast.MAP.get(IL).visit(this, frame);
      if (branchSize > maxBranchSize) {
        maxBranchSize = branchSize;
      }

      emit(Machine.JUMPop, 0, Machine.CBr, switchAddr);
      patch(jumpAddr, nextInstrAddr);
    }

    // Manejo del caso "por defecto" si corresponde.
    emit(Machine.LOADop, 1, Machine.STr, -1);
    int jumpEndAddr = nextInstrAddr;
    emit(Machine.JUMPIFop, Machine.trueRep, Machine.CBr, jumpEndAddr);
    ...
    // Retornar el tamaño máximo calculado.
  ...
  public Object visitRecordTypeDenoter(RecordTypeDenoter ast, Object o) {
    int typeSize;
    if (ast.entity == null) {
      typeSize = ((Integer) ast.FT.visit(this, 0)).intValue();
      ast.entity = new TypeRepresentation(typeSize);
      writeTableDetails(ast);
    } else
      typeSize = ast.entity.size;
